    ...
    function getUniqueFilteredNodes(currentNode, nodes, links) {
      ...
      const connectedNodeIds = [
        ...
        ...links
          ...
          .map((link) => link.source), // Add nodes connected through incoming links
      ];
      const filteredNodes = [currentNodeObj, ...nodes.filter((node) => connectedNodeIds.includes(node.id))];
    ...
    function redrawGraph(currentNode) {
      ...
      .call(
        ...
        node.on("click", (event, d) => {
          ...
          node.attr("r", (nodeData) => (nodeData.id === currentNode ? nodeData.size * 1.3 : nodeData.size));
